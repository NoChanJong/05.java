...
/*
	...
	라이브러리를 추가하는 방법
	1. Java프로젝트를 우클릭
	2. Build Path > Configure Build Path > (tab)Libraries > Add Jars(or External Jars) >  Apply & Close
	...
	4. Query를 실행
	   ...
	   executeQuery() : 수행결과로 ResultSet객체의 값을 반환, select구문을 수행할 때 사용되는 함수
	   
	   ResultSet rs = pstmt.executeUpdate("select * from emp where empno = ?");
	   executeUpdate() : insert, delete, update 관련 구문에세는 반영된 레코드의 건수를 반환,
	   					 create/drop 관련 구문에서는 -1을 반환
	   
	   ResultSet rs = cstmt.execute(); 
	   execute() : 수행결과로 Boolean타입의 값을 반환, 모든 구문을 수행할 수 있다.
	...
	5. ResultSet처리 
	   ...
	   데이터를 추출하는 방법은 ResultSet에서 한 행(row)씩 이동하면서 getXXX()메서드를 이용하여 데이터를 
	...
	1. DriverManager 클래스
	   ...
	   를 찾지 못했을 경우에는 forName()메서드는 ClassNotFoundException예외를 발생시키기 때문에 반드시 에외
	   처리를 해야 한다. 
	...
	2. Connection 인터페이스
	   ...
	   을 실행시키기 전에 우선 Connection객체가 있어야 한다. Connection객체는 특정 데이터 원본과 연결된 커넥션
	...
	4. PreparedStatement 인터페이스
	   ...
	   PreparedStatement객체는 각각의 SQL타입을 처리할 수 있는 setXXX()메서드를 제공한다. 여기서 XXX는 해당
	   테이블의 데이터타입과 관련이 있다. 해당 필드의 데이터타입이 문자열이면 setString()이 되고 숫자값일 
	   ...
	   3) Statement객체는 쿼리실행시 작은 따옴표(')가 있으면 작은따옴표 2개로 표시해야하는 번거로움이 있
	...
	6. ResultSet 인터페이스
	   ...
	   커서는 초기에 첫번째 행의 바로 앞을 가리키도록 되어 있는데 ResultSet객체의 next()메서드를 이용해서
	   다음 행으로 이동할 수 있다.
	   ...
	   3) beforeFirst() : 커서를 첫번째 행 이전으로 이동
	   4) afterLast()   : 커서를 마지막 행 이후로 이동
	   5) next()		: 커서를 다음행으로 이동
	   6) previous()	: 커서를 이전행으로 이동
	   
	   ResultSet에서 행을 처리하는 데 반복문을 사용하며 next()메서드가 유효한 행이 있으면 true, 없으면 false
	...
	7. JDBC 트랜젝션
	   ...
	   종료가 되면 commit이 되고 하나라도 문제가 있다면 rollback이 되어서 작업수행전으로 되돌린다.
