...
public class BoardDAOImpl implements BoardDAOService {
	...
	public BoardVO viewBoard(int bno) {
		...
		try {
			...
			System.out.println("선택한 글번호 글목록 조회 실패.");
			e.printStackTrace();
		} finally {
			try {
				if(rs != null) rs.close();
		...
		return board; // BoardMenu파일에 contentView메서드 BoardVO생성자에 리턴
	}
	...
	public void updateBoard(int bno) {
		...
		try {
			...
			int row = pstmt.executeUpdate();
			System.out.println("board테이블에서 " + row + "건이 성공적으로 수정되었습니다!");
		} catch (Exception e) {
			// TODO: handle exception
		} finally {
			try {
	...
	public void deleteBoard(int bno) {
		...
		try {
			...
			int row = pstmt.executeUpdate();
			System.out.println("board테이블에서 " + row + "건이 성공적으로 삭제되었습니다!");
		} catch (Exception e) {
			// TODO: handle exception
		} finally {
			try {
