...
/*
	...
	문자열을 저장하는 String클래스는 내부의 문자열을 직접 수정할 수 없다. 예를 들어 String.replace()
	메서드는 내부의 문자열을 변경하는것이 아니라 새롭게 대체된 문자열객체를 리턴한다. 또한 예를들어
	"JAVA" + "PROGRAMMING"을 실행하는 경우 내부문자열을 수정할 수 없기 때문에 "JAVA PROGRAMMING"이라는 
	새로운 String객체를 생성한 후에 그 객체의 참조주소를 리턴한다. 문자열을 결합할 경우에 "+"연산자를 
	사용하면 그 수 만큼의 새로운 String객체의 수가 증가하기 때문에 프로그램의 성능저하의 원인이 된다.
	...
	자동으로 버퍼크기만큼 증가시켜 주기 때문에 초기버퍼의 크기는 그다지 중요하지가 
	않다.
