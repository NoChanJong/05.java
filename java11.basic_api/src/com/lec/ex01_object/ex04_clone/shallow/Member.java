...
/*
	...
	신뢰하지 않은 영역으로 원본객체를 넘겨서 작없을 할 경우 원본객체의 데이터가 훼손될 수 
	있기 때문에 복제된 객체를 만들어 신뢰하지 않는 영역으로 넘기는 것이 복제된 객체의 데이터
	가 변경되더라도 원본객체는 아무런 영향을 받지 않기 때문에 안전하게 데이터를 보호할 수 
	있게 된다.
	
	Object.clone()메서드는 자신과 동일한 필드값을 가진 얕은 복사된 객체를 리턴한다.
	clone()메서드로 객체를 복제하려면 원본객체는 반드시 java.lang.Cloneable인터페이스를
	구현하고 있어야 한다. 클래스설계자가 복제허용하지 않도록 한다면 Cloneable인터페이스를 
	구현하지 않으면 된다.
	
	객체를 복제하는 방법은 얕은복제와 깊은 복제가 있다.
	
	1. 얕은복사(shallow clone)
	...
	2. 깊은복사(deep clone)
	   ...
	   객체의 필드는 같은 메모리 주소를 참조한 동일 객체를 참조하게 된다. 만일, 복제객체에
