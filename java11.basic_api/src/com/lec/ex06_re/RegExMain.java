...
/*
	...
	간단히 말해서 정규식은 문자 or 숫자등 반복기호가 결합된 문자열이다. 예를 들어 문자열이
	02-123-1234 또는 010-1234-1234와 같이 전화번호를 포함하는지 여부를 확인할 수가 있는데
	정규식으로 표현하면 "(02|010)-\d{3,4}-\d{4}"의 형태인지 여부를 boolean으로 리턴, 이메일일
	경우 "hong@gmail.com"의 패턴은 "\w+@\w+\.\w(\.\w+)?"의 형태로 검증할 수가 있다.
 */
public class RegExMain {
	...
	public static void main(String[] args) {
		...
		if(Pattern.matches(re, email)) {
			...
			System.out.println("비정상적인 이메일입니다.");
		}
		...
		System.out.println(Pattern.matches("[A-Z]", "A"));   // only 대문자 true
		System.out.println(Pattern.matches("[a-z]", "az"));  // 개별문자 false
		System.out.println(Pattern.matches("[a-z]+", "az")); // + 단어단위, true
		System.out.println(Pattern.matches("[A-Z]+", "AZ")); // + 단어단위, true
		System.out.println(Pattern.matches("[A-Z]+", "Az")); // + 단어단위, false
		System.out.println(Pattern.matches("[a-zA-Z]+", "Az")); // + 단어단위, 소문자, 대문자, true
		System.out.println();
		...
		// 영문자, 소문자, 대문자, _
		System.out.println(Pattern.matches("[0-9a-zA-Z]+", "01a9Eb"));       // true
