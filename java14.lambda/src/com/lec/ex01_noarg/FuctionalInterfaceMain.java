...
/*
	...
	자바는 함수형프로그램을 위해 Java8버전부터 람다식을 지원하면서 기존의 코딩 패턴이 많이 달라 졌다.
	람다식은 수학자 알론조 처리(Alonzo Church)가 발표한 계산법에서 사용된 수학식의 표현으로 이를 제자인
	존 메카시(Jon McCarthy)가 프로그래밍언어에 도입했다.
	
	람다식은 익명함수를 생성하기 위한 식으로 객체지향언어보다 함수지향언어에 가깝다. 자바에 람다식을
	...
	1. 기본문법
	   ...
	   타입을 일반적으로 정의하지 않는다. 예를 들어 매개변수가 하나가 있을 경우에는 아래와 같이 정의할 수 
	   있다. 
	   
	      (int a) -> { System.out.prinln(a); }
	   or a -> { System.out.prinln(a); }  : 소괄호가 없는 경우는 매개변수가 하나만 있을 경우에만 생략가능
	   or a -> System.out.prinln(a);	  : 중괄호가 없는 경우는 실행문장이 하나만 있을 경우에만 생략가능
	...
	2. 타겟함수와 함수적인터페이스
	   ...
	   페이스는 직접 객체생성을 할 수 없기 때문에 구현클래스가 필요한데 람다식은 구현하는 클래스 선언없이
	...
	3. 함수적인터페이스(Functional Interface)
	   ...
	   모든 인터페이스를 람다식의 타겟타입으로 사용할 수 없다. 람다식이 하나의 메서드만 정의하기 때문에 
	   ...
	   가 선언되지 않도록 컴파일러가 확인할 수 있게 하기 위해서는 명시적으로 @FunctionalInterface 애너
	   테이션을 선언해야 한다.	  
*/
public class FuctionalInterfaceMain {
