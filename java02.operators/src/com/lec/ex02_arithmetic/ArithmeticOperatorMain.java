...
/*
 	...
 	산술연산자는 사칙연산과 나머지를 구하는 연사자를 포함해서 총 5개의 연산자가 있다.
 	...
 	두 피연사자의 타입 중 더 큰 타입으로 일치시키는데, 그 이유는 작은 타입으로 형변환하면
 	원래의 값이 손실될 가능성이 있기 때문이다.
 	...
 	3. 피연산자중 실수타입(float, double)이 있을 경우 크기가 큰 실수 타입으로 변환후 연산을 수행
 	   ...
 	   int + double -> double + double = 결과는 double이다.
