...
/*
	...
	부모타입의 변수가 부모객체를 참조할 경우 자식타입으로 변환할 수가 있다. 예를들어
	...
	instanceof연산자는 매개값의 타입을 조사할 때 주로 사용한다. 메서드내에서 강제타입변환이 필요한 경우
	반드시 매개값이 어떤 타입인지를 instanceof연산자로 확인하고 안전하게 강제타입변환을 해야 한다.
	
	만약 타입을 확인하지 않고 강제타입변환을 시도할 경우에 강제타입변환이 되지 않을 경우에는 ClassCastException예외가 발생한다.
...
public class InstanceofMain {
	...
	private static void method2(Parent instance) {
		...
		if(instance instanceof Child) {
			Child child = (Child) instance; 
			System.out.println("method2() - 강제형변환성공");
		}
	}
