...
/*
	...
	이 OptionalXXX클래스들은 저장한느 값의 타입만 다를 뿐 제공하는 기능은 거의 유사하다.
...
public class AggregateMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("2배스의 평균 = " + result1);
		System.out.println();
		
		// 4. max
		System.out.println(Arrays.stream(int_arr).filter(n -> n%2 == 0).max().getClass());
		System.out.println("2배수의 최대값 = " + Arrays.stream(int_arr).filter(n -> n%2 == 0).max().getAsInt());
		System.out.println();
		
		// 5. min
		System.out.println(Arrays.stream(int_arr).filter(n -> n%2 == 0).min().getClass());
		System.out.println("2배수의 최소값 = " + Arrays.stream(int_arr).filter(n -> n%2 == 0).min().getAsInt());
		System.out.println();

		// 6. findFirst
		System.out.println(Arrays.stream(int_arr).filter(n -> n%2 == 0).findFirst().getClass());
		System.out.println("2배수중 첫번쨰 값 = " + Arrays.stream(int_arr).filter(n -> n%2 == 0).findFirst().getAsInt());
		System.out.println();
