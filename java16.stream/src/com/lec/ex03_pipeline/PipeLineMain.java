...
/*
	...
	평균, 카운팅, 최대/최소값등이 대표적인 reduction의 결과이다. 그러나 컬렉션의 요소를 리덕션의 결과
	물로 바로 집계할 수 없을 경우에는 집계하기 편리하도록 필터링, 매핑, 그룹핑등의 중간처리를 하는
	것이 필요하다. 
	...
	1. 중간처리와 최종처리
	   ...
	   중간스트림이 생성될 때 요소들이 바로 중간처리되는 것이 아니라 최종처리가 시작되기 전까지는
	...
	2. 중간처리와 최종처리 메서드
	   ...
	   1) 중간처리메서드
	      ...
	      b. 맵핑   : flatMap(), flatMapToDouble(), flatMapToLong(), flatMapToInt()
...
public class PipeLineMain {
	...
	public static void main(String[] args) {
		...
		OptionalDouble avgAge = members.stream()
									   .filter(m -> m.getGender() == Member.MALE) // 남자회원만 stream으로 다음(mapToInt())으로 전달
									   .mapToInt(Member::getAge)				  // Member객체 "홍길동, 30" -> stream(나이=30)만 다음(average())으로 전달
									   .average(); // 최종값(average() = double)  // stream [30, 28, 35] -> 평균(Double)
		System.out.println("stream로직 : 남자회원의 평균나이 = " + avgAge.getAsDouble());
		System.out.println();
		...
		double avg = members.stream()
							...
							.average()
							.getAsDouble();
		System.out.println("stream로직 : 여자회원의 평균나이 = " +avg);
		System.out.println();
	}
